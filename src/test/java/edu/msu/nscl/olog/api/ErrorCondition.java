...
public class ErrorCondition {
	...
	public void addOrphanLog() {
		...
		client.update(log(xmlLog.getSubject()).level("Info"));
	}

	@Test(expected = OlogException.class)
	public void addOrphanLogbook() {
		client.set(logbook("JustName"));
	}
	...
	public void addSetwithBadLog() {
		...
		logs.add(log("name1").description("some details").level("Info")
				.in(logbook("book")));
		logs.add(log("").in(logbook("book")));
		// log ch2 has empty name
		try {
			client.set(logbook("book"));
			client.set(logs);
			assertTrue(false);
		} catch (OlogException e) {
			assertTrue(true);
	...
	/**
	 * Test detection of error condition - attempting to update a log that does
	 * not exist
	 */
	...
	public void addTag2NonExistentLog() {
		Long nonExistingId = 0L;
		try {
			client.set(tag("sometag"), nonExistingId);
			assertTrue(false);
		} catch (OlogException e) {
			assertTrue(true);
		}
