...
public class LogBuilder {
		...
		public static LogBuilder log(Log log) {
			...
			for (Logbook logbook : log.getLogbooks()) {
				logBuilder.logbooks.add(LogbookBuilder
						.logbook(logbook));
			}
                        for (Attachment attachment : log.getAttachments()) {
                                logBuilder.attachments.add(AttachmentBuilder
                                                .attachment(attachment));
                        }
                        for (Property property : log.getProperties()) {
                                logBuilder.properties.add(PropertyBuilder
                                                .property(property));
                        }
			return logBuilder;
		}
		...
		public LogBuilder in(LogbookBuilder logbook) {
			...
			return this;
                ...
                @Deprecated public LogBuilder attach(AttachmentBuilder attachment) {
